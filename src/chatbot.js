document.addEventListener("DOMContentLoaded", () => {
    ...
    /*
        This stops the script from breaking on pages
        where the chatbot HTML has not been added.
    */
    if (
        !chatbotToggle ||
        !chatbotPanel ||
        ...
        console.warn("Chatbot HTML not found on this page.");
        return;
    }
    ...
    function showMainOptions() {
        ...
        showOptions([
            ...
            {
                label: "Payment or billing help",
                action: "billingHelp"
            },
            {
                label: "Existing booking help",
    ...
    async function handleOption(action, value = null) {
        ...
        if (action === "billingHelp") {
            ...
            addMessage(
                "Please enter your account email address so I can check billing information.",
                "bot"
            );
        ...
        if (action === "existingBooking") {
            ...
            addMessage(
                "Please paste your booking management token from your confirmation email.",
                "bot"
            );
        ...
        if (action === "chooseProvider") {
            ...
            addMessage(
                "Please enter your preferred appointment date in YYYY-MM-DD format. Example: 2026-05-28",
                "bot"
            );
    ...
    async function loadSlots() {
        try {
            ...
            const res = await fetch(
                `${CHATBOT_API_BASE}/api/booking-slots?${params.toString()}`
            );
            ...
            if (!data.slots || !data.slots.length) {
                addMessage(
                    "No available slots found for this date. Please enter another date.",
                    "bot"
                );
            ...
            addMessage("Could not load slots. Please try another date.", "bot");
    ...
    async function handleTypedInput(text) {
        ...
        if (currentStep === "bookingEmail") {
            ...
            bookingData.email = text.trim().toLowerCase();
        ...
        if (currentStep === "billingEmail") {
            ...
            await loadBilling(text.trim().toLowerCase());
            return;
        }
        ...
        if (currentStep === "contactEmail") {
            ...
            contactData.email = text.trim().toLowerCase();
    ...
    async function loadExistingBooking(token) {
        try {
            const res = await fetch(
                `${CHATBOT_API_BASE}/api/booking/manage/${encodeURIComponent(token)}`
            );
    ...
    function showServiceDetails(service) {
        const details = {
            "Tax Filing & Compliance":
                "We help with tax returns, lodgements, amendments, and ATO-compliant records.",
            "Business Consulting":
                "We help with cash flow, KPI dashboards, business structure, and growth advice.",
            "Payroll":
                "We support STP, super, awards, onboarding, and monthly payroll runs.",
            "Auditing":
                "We provide independent assurance for compliance and financial confidence."
        };

        addMessage(
            details[service] || "This service is available at Constant & Co.",
            "bot"
        );
