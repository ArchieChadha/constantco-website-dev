document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (paymentServiceEl) {
            paymentServiceEl.textContent = formatText(chargeData.serviceName || 'Not assigned');
        }

        if (paymentBookingFeeEl) {
            paymentBookingFeeEl.textContent = `$${(Number(chargeData.bookingFee || 0) / 100).toFixed(2)} AUD`;
        }

        if (paymentServiceChargeEl) {
            paymentServiceChargeEl.textContent = `$${(Number(chargeData.serviceCharge || 0) / 100).toFixed(2)} AUD`;
        }

        if (paymentStatusEl) {
            paymentStatusEl.textContent = formatText(chargeData.status || 'Pending');
        }

        if (paymentTotalDueEl) {
            paymentTotalDueEl.textContent = `$${(Number(chargeData.totalDue || 0) / 100).toFixed(2)} AUD`;
        }
